...
import com.marry.marry.config.SystemConfig;
...
public class ServiceManager {
  ...
  /**
   * @notice: must use @Qualifier("ProofService") with @Autowired to get this Bean
   */
  @Bean("ProofService")
  public Map<String, ProofService> initProofServiceManager() throws Exception {
    Map<String, ProofService> serviceMap = new ConcurrentHashMap<>(this.hexPrivateKeyList.size());
    for (int i = 0; i < this.hexPrivateKeyList.size(); i++) {
    	...
    	proofService.setAddress(this.config.getContract().getProofAddress());
    	proofService.setClient(this.client);
    	...
    	proofService.setTxProcessor(txProcessor);
    	serviceMap.put(userAddress, proofService);
    }
    log.info("++++++++ProofService map:{}", serviceMap);
    return serviceMap;
  }
}
